...
func (c *Equal) Compare(left, right []reflect.Value) bool {
	kind := getKind(left, right)
	compare := c.compares[kind]
	if compare == nil {
		panic("No Equal Compare found for kind:" + kind.String())
	}
	for _, aside := range left {
		for _, zside := range right {
			if compare(aside, zside) {
				return true
			}
		}
	}
...
func eqStringMatcher(left, right reflect.Value) bool {
	aside := strings.ToLower(left.String())
	zside := strings.ToLower(right.String())
	return aside == zside
}
